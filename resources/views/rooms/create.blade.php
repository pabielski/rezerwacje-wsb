...
@section('content')
    ...
    <form action="/rooms/add-to-database" method="post" class="bg-surface border border-border rounded-2xl shadow-sm p-6 max-w-xl space-y-4">
        @csrf
        ...
        <div><label class="block text-sm font-semibold text-heading mb-1.5">ID hotelu</label><input type="text" name="hotel_id" value="{{ old('hotel_id', $room->hotel_id) }}" class="w-full border border-border rounded-lg px-3 py-2.5 text-sm focus:outline-none focus:border-brand focus:ring-2 focus:ring-brand/20"></div>
