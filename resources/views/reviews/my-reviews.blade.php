...
@endif
    <div class="flex flex-wrap items-center justify-between gap-4 mb-6">
        <div>
            ...
            <p class="text-sm text-muted mt-1">Opinie dodane przez Ciebie</p>
        </div>
    ...
    <form action="/my-reviews" method="get" class="bg-surface border border-border rounded-2xl shadow-sm p-5 mb-6">
        ...
        <a href="/my-reviews" class="text-muted hover:text-heading text-sm font-medium ml-3">Wyczyść filtry</a>
    </form>
    <div class="bg-surface border border-border rounded-2xl shadow-sm overflow-hidden">
        @forelse ($reviews as $review)
            <div class="p-5 border-b border-border last:border-b-0 hover:bg-page/50 transition-colors">
                <div class="flex flex-wrap justify-between gap-2">
                    <h2 class="text-lg font-bold text-heading">Rezerwacja #{{ $review->reservation_id }}</h2>
                    <span class="px-2.5 py-0.5 rounded-md text-xs font-semibold bg-page text-muted">{{ $review->rating }}/5</span>
                </div>
                <p class="text-muted text-sm mt-1">{{ $review->created_at }}</p>
                <p class="text-heading text-sm mt-2">{{ $review->content }}</p>
        ...
        @empty
            <p class="p-8 text-center text-muted">Nie dodałeś jeszcze żadnej opinii.</p>
        @endforelse
    </div>
@endsection
