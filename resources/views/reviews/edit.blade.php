...
@section('content')
    ...
    <form action="/reviews/update/{{ $review->id }}" method="post" class="bg-surface border border-border rounded-2xl shadow-sm p-6 max-w-xl space-y-4">
        @csrf
        <div>
            ...
            <select name="reservation_id" class="w-full border border-border rounded-lg px-3 py-2.5 text-sm focus:outline-none focus:border-brand focus:ring-2 focus:ring-brand/20">
                ...
                @foreach ($reservations as $reservation)
                    <option value="{{ $reservation->id }}" @selected(old('reservation_id', $review->reservation_id) == $reservation->id)>
...
<div>
        ...
        <div><label class="block text-sm font-semibold text-heading mb-1.5">Treść opinii</label><textarea name="content" rows="4" class="w-full border border-border rounded-lg px-3 py-2.5 text-sm focus:outline-none focus:border-brand focus:ring-2 focus:ring-brand/20">{{ old('content', $review->content) }}</textarea></div>
