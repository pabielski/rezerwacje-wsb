...
<form action="/my-reservations/add-to-database" method="post" class="bg-surface border border-border rounded-2xl shadow-sm p-6 max-w-xl space-y-4">
    @csrf
    <div><label class="block text-sm font-semibold text-heading mb-1.5">Pokój</label><select name="room_id" class="w-full border border-border rounded-lg px-3 py-2.5 text-sm focus:outline-none focus:border-brand focus:ring-2 focus:ring-brand/20">
            <option value="">Wybierz pokój</option>@foreach ($rooms as $room)<option value="{{ $room->id }}" @selected(old('room_id')==$room->id)>{{ $room->name }} (nr {{ $room->room_number }}) — {{ $room->price_per_night }} zł/noc</option>@endforeach
        </select></div>
